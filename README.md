# Superstore Sales Analysis

This project analyses retail sales data using Microsoft Excel.

Skills Used:
- Data Cleaning
- Pivot Tables
- Dashboard Creation
- Data Visualisation

Key Business Insights

1. High Revenue Product Category
The Technology category generated the highest sales revenue among the categories. This indicates strong customer demand for technology products.
Business recommendation: The company should prioritise marketing and inventory planning for high-performing categories to maximise revenue.

2. Regional Sales Performance
The West region recorded the highest sales performance among all regions.
Business recommendation: The company can analyse strategies used in the West region and apply similar approaches in lower-performing regions to improve sales.

3. High Sales but Low Profit Products
Some products generate high sales but lower profit margins, which may indicate heavy discounting or high operational costs.
Business recommendation: The company should review discount policies and operational costs to improve profitability.

4. Seasonal Sales Trends
Sales fluctuate across months, with certain periods showing higher demand.
Business recommendation: Businesses should plan inventory and marketing campaigns around peak sales periods

5. Customer Contribution Analysis
A small group of customers contributes significantly to overall sales.
Business recommendation: Implement loyalty programs and targeted marketing campaigns to retain high-value customers
